...
function LinuxIcon({ size = 22 }: { size?: number }) {
  return (
    //<svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    //  <path d="M12.504 0c-.155 0-.315.008-.48.021-4.226.333-3.105 4.807-3.17 6.298-.076 1.092-.3 1.953-1.05 3.02-.885 1.051-2.127 2.75-2.716 4.521-.278.832-.41 1.684-.287 2.489a.424.424 0 00-.11.135c-.26.268-.45.6-.663.839-.199.199-.485.267-.797.4-.313.136-.658.269-.864.68-.09.189-.136.394-.132.602 0 .199.027.4.055.536.058.399.116.728.04.97-.249.68-.28 1.145-.106 1.484.174.334.535.47.94.601.81.2 1.91.135 2.774.6.926.466 1.866.67 2.616.47.526-.116.97-.464 1.208-.946.587-.003 1.23-.269 2.26-.334.699-.058 1.574.267 2.577.2.025.134.063.198.114.333l.003.003c.391.778 1.113 1.368 1.884 1.43.868.07 1.723-.26 2.456-.594.733-.34 1.455-.678 2.186-.78.714-.098 1.315.066 1.878.348.566.283 1.097.625 1.501.556.403-.073.64-.456.801-.948.162-.498.257-1.084.399-1.562.142-.479.373-.836.795-1.14.516-.371.886-.79 1.164-1.175a3.99 3.99 0 00.6-1.377c.071-.334.1-.672.1-.99 0-.178-.004-.347-.013-.507-.012-.211-.04-.403-.075-.586a2.5 2.5 0 00-.394-.993c-.291-.46-.673-.834-1.073-1.168-.4-.333-.82-.64-1.175-.903-.355-.264-.656-.49-.801-.725a1.053 1.053 0 01-.154-.396 1.05 1.05 0 01-.013-.263c.004-.134.03-.262.054-.386.048-.252.112-.503.127-.726.012-.11.013-.214.009-.31a1.047 1.047 0 00-.06-.312c-.065-.165-.158-.258-.254-.348-.096-.089-.198-.159-.3-.229a7.395 7.395 0 00-.633-.36c-.469-.243-1.036-.469-1.588-.87-.552-.4-1.09-.96-1.373-1.79-.283-.83-.174-1.468-.064-2.107.055-.32.113-.64.142-.972.029-.332.029-.672-.03-1.027-.058-.354-.173-.717-.39-1.092-.434-.75-1.248-1.368-2.353-1.572a5.763 5.763 0 00-.467-.065 5.56 5.56 0 00-.52-.021z" />
    //</svg>
    <svg
      width={size}
      height={size}
  ...
  return (
    <ul className="space-y-4" aria-label="Available downloads">
      {downloads.map((dl) => (
        <li
          key={dl.id}
          className="flex items-center gap-4 p-5 rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] hover:border-[var(--accent)]/30 transition-colors"
        >
          <div
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 mt-1.5 text-xs text-[var(--text-secondary)]">
              <span>
                {t("version")} {dl.version}
              </span>
              <span>
                {t(dl.platform === "all" ? "allPlatforms" : dl.platform)}
              </span>
          ...
          <Button
            ...
            disabled={loadingId === dl.id}
            aria-label={`${t("download")} ${dl.title} v${dl.version}`}
          >
            <DownloadIcon size={16} className="mr-2" />
            {loadingId === dl.id ? t("preparing") : t("download")}
          </Button>
        </li>
      ))}
    </ul>
